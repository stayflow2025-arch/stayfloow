...
import { properties, cars, circuits } from "@/lib/data";
import { bookings } from "@/lib/bookings-data";
import { useCurrency } from "@/context/currency-context";
import { CircleDollarSign, Home, TrendingUp, Target, Award, Eye, Percent, Calendar, MessageSquare, Building, Car as CarIcon, Compass, Star } from "lucide-react";
import { Button } from "@/components/ui/button";

const partnerId = "mock-partner-1"; // In a real app, this would come from the user session

// Mock data for the dashboard
const partnerListings = [
    ...properties.slice(1, 3).map(p => ({...p, itemType: 'Hébergement'})),
    ...cars.slice(0, 1).map(c => ({...c, itemType: 'Véhicule', name: `${c.make} ${c.model}`})),
];

const partnerBookings = bookings.slice(0, 2);

const totalEarnings = partnerBookings.reduce((acc, booking) => acc + booking.totalAmount * 0.8, 0); // Assuming 20% commission
                            ...
                            {partnerBookings.map(booking => (
                                <div key={booking.id} className="flex items-center justify-between p-3 bg-secondary rounded-lg">
                                    <div>
                                        ...
                                        <p className="text-sm text-muted-foreground">{booking.customer.name} - {booking.dates}</p>
                                    </div>
