    ...
    try {
      ...
      if (newFavoritedState) {
        ...
        await sendFavoriteReminderEmail({
            customerName: "Cher Voyageur", // Placeholder name
            customerEmail: "stayflow2025@gmail.com", // Placeholder email
            property,
        });
                    ...
                    <div>
                        <h3 className="font-headline text-2xl font-bold leading-tight">
                         ...
                         <div className="flex items-center gap-2 mt-1">
                             <Link href={`/search?location=${property.location}`} className="text-sm text-muted-foreground hover:text-primary flex items-center gap-1">
                     ...
                     <div className="flex items-center gap-2 text-right">
                        ...
                        <div className={cn("flex items-center justify-center h-10 w-10 text-white font-bold rounded-md text-lg", getRatingColor(property.rating))}>
    ...
    <Card className="overflow-hidden flex flex-col h-full transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
      <div className="relative group">
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-2 z-10">
          {property.isBoosted && (
              <Badge className="bg-amber-400 text-amber-900 shadow-md border-amber-500 hover:bg-amber-400">
                  <Zap className="w-3 h-3 mr-1" />
                  Boosté
              </Badge>
          )}
          {property.isWeekendOffer && (
              <Badge className="bg-violet-500 text-white shadow-md border-violet-600 hover:bg-violet-500">
      ...
      <CardContent className="p-4 pt-0">
        <div>
            {isGenius && (
             ...
             <p className={cn("font-bold", isGenius ? "text-sm text-muted-foreground line-through" : "text-lg")}>
                {formatPrice(property.price)}{" "}
            </p>
             <p className="text-sm font-normal text-muted-foreground -mt-1">/ nuit</p>
        </div>
      </CardContent>
    </Card>
  );
