...
export function CarFilterSidebar({ resultCount }: { resultCount: number }) {
  ...
  const [priceRange, setPriceRange] = useState([convertFromDZD(2000), convertFromDZD(15000)]);
  ...
  return (
    <Card className="sticky top-24">
      ...
      <CardContent className="p-0 max-h-[calc(100vh-12rem)] overflow-y-auto">
        <Accordion type="multiple" defaultValue={['budget', 'type']} className="w-full">
          
          <AccordionItem value="budget">
            <AccordionTrigger className="p-4 font-semibold">Budget (par jour)</AccordionTrigger>
          ...
          <AccordionItem value="type">
            <AccordionTrigger className="p-4 font-semibold">Type de véhicule</AccordionTrigger>
          ...
          <AccordionItem value="transmission">
            <AccordionTrigger className="p-4 font-semibold">Boîte de vitesses</AccordionTrigger>
            <AccordionContent className="p-4 pt-0 space-y-3">
              {['Manuelle', 'Automatique'].map(item => (
                <div key={item} className="flex items-center space-x-2">
                  <Checkbox id={`trans-${item}`} />
                  <Label htmlFor={`trans-${item}`} className="font-normal">{item}</Label>
                </div>
              ))}
            </AccordionContent>
          </AccordionItem>
          ...
          <AccordionItem value="fuel">
            <AccordionTrigger className="p-4 font-semibold">Type de carburant</AccordionTrigger>
            <AccordionContent className="p-4 pt-0 space-y-3">
              {fuelTypes.map(item => (
                <div key={item} className="flex items-center space-x-2">
                  <Checkbox id={`fuel-${item}`} />
                  <Label htmlFor={`fuel-${item}`} className="font-normal">{item}</Label>
                </div>
              ))}
            </AccordionContent>
          </AccordionItem>
           ...
           <AccordionItem value="features">
            <AccordionTrigger className="p-4 font-semibold">Équipements</AccordionTrigger>
            <AccordionContent className="p-4 pt-0 space-y-3">
