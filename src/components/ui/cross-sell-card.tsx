...
import { ArrowRight, Car, Compass, BedDouble, Calendar as CalendarIcon, Users } from "lucide-react";
...
export function CrossSellCard({ location, bookedItemType }: { location: string, bookedItemType?: BookedItemType }) {
    ...
    const suggestions = [
        ...
        },
        {
            ...
            description: 'Laissez-vous guider par des experts locaux pour une expérience immersive et inoubliable.',
            href: '/circuits',
    ...
    const filteredSuggestions = suggestions.filter(s => s.type !== bookedItemType);
    ...
    return (
        <div className="space-y-6">
             <h2 className="font-headline text-3xl font-bold">Complétez Votre Séjour à {location}</h2>
             <p className="text-muted-foreground max-w-2xl mx-auto">Maintenant que votre réservation est confirmée, pourquoi ne pas explorer les environs ?</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredSuggestions.map(suggestion => {
                    const Icon = suggestion.icon;
                    return (
                        <Card key={suggestion.type} className="overflow-hidden group text-left">
                            ...
                            <CardContent className="p-6">
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <h3 className="font-headline text-2xl font-bold">{suggestion.title}</h3>
                                </div>
                                                ...
                                                <PopoverTrigger asChild>
                                                    ...
                                                    >
                                                        ...
                                                        {circuitDates?.from ? (
                                                            circuitDates.to ? (
                                                                <>
                                                                    {format(circuitDates.from, "dd LLL y", { locale: fr })} - {format(circuitDates.to, "dd LLL y", { locale: fr })}
                                                                </>
                                                            ) : format(circuitDates.from, "dd LLL y", { locale: fr })
                                                        ) : (
